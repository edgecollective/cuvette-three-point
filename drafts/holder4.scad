// holder4.scad -- light-tight version of holder3 (self-contained).
//
// Instead of holder3's open corner notch, the chamber walls are thickened
// (wall_width 2 -> 6) so the clip tongue lives in a fully INTERNAL pocket:
// sealed on every side by solid material, opening only through the narrow
// slit at the cavity corner that the nub needs to reach the cuvette.  No
// ambient light path into the chamber remains, and 6 mm walls are opaque
// where 2 mm printed PLA is not.  Spring geometry is identical to holder2/3.
//
// PRINT UPSIDE DOWN (top face on the bed): every face up there is flush, the
// tongue then builds base-first from its anchor block with no support, and
// the enclosed pocket needs none.  Only the chamber floor bridges ~13 mm
// (fine), and the shroud's far segment bridges ~35 mm (enable supports there
// or accept sag; it's non-optical).
...
clip_root_z  = 36;    // pocket ceiling; tongue anchors into solid above
            ...
            for (a=[0:90:270])
                rotate([0,0,a])
                    translate([corner_flat_d/sqrt(2), corner_flat_d/sqrt(2), 0])
                        rotate([0,0,45])
                            translate([0, -outer_side, -(holder_depth+2)/2])
...
/* ---------- shrouded PCB mounting plate (as holder3) ---------- */
module pcb_mount() {
    difference() {
        union() {

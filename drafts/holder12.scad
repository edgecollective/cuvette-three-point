// holder12.scad -- holder11 with nut-access shaves on the +/-y walls.
//
// New in holder12: placing nuts on the inner side of the PCB plate holes
// was tight.  The +/-y chamber walls are shaved down to nut_shave_wall
// (3 mm) thick ABOVE the spring pocket's sealed ceiling (shave starts at
// pocket_top + pocket_skin, so the pocket keeps its opaque roof), opening
// a full-length slot between each plate and the chamber to drop nuts in
// from above and slide them onto the upper screws.  The lower holes sit
// below the pocket ceiling and keep only the original chamfer clearance --
// the +y wall can't be thinned there without exposing the spring chamber
// (which is what a global wall_width of 4 does).
//
...
nub_w        = 3.0;   // width of the flat block face across the corner
nub_z_lo     = 12;    // bottom of the flat pressing face
...
/* ---- nut-access shave ---- */
nut_shave_wall = 3;   // +/-y wall thickness left above the shave line
nut_shave_z    = pocket_top + pocket_skin;  // shave starts above the
                                            // pocket's sealed roof
...
module body() {
    difference() {
        ...
        // nut-access shave: thin the +/-y walls above the pocket's sealed
        // roof, opening a slot beside each plate for dropping nuts onto
        // the upper screws
        for (s=[-1,1])
            translate([-outer_side/2 - 1,
                       s == 1 ? holder_side/2 + nut_shave_wall : -outer_side/2 - 1,
                       nut_shave_z])
                cube([outer_side + 2,
                      wall_width - nut_shave_wall + 1,
                      chamber_depth - nut_shave_z + 1]);
    }
...
module tongue() {
    ...
    // matching chamfer underneath makes it print cleanly right side up.
    ...
    at_corner(d_face)
        rotate([90, 0, 0])
            linear_extrude(height=nub_w, center=true)
                polygon([[-0.5, nub_z_lo - nub_protr - 0.5],   // back, bottom

// holder10.scad -- holder9 with a bottom-hinged spring and a bigger nub,
// printable RIGHT SIDE UP.
//
// New in holder10 (from print testing of holder9 -- cuvette sat too loose):
...
nub_r        = 2.0;   // fat nub: prints reliably, gentle insertion ramp
nub_len      = 3;
nub_z        = window_z;  // nub presses at optical-path height

/* ---- internal clip pocket (diagonal distances from cavity center) ---- */
pocket_front = 8.3;   // front plane; breaks the cavity corner into a slit
                      // (deepened so the r=2 nub clears the slit edges)
...
module tongue() {
    ...
    // cuvette corner, round front ramps for insertion and removal.  The fat
    // (r=2) nub is flattened flush with the tongue's back face so it doesn't
    // protrude behind the tongue and eat the spring-back travel.
    at_corner(d_nub_tip + nub_r)
        intersection() {
            ...
            translate([-(d_face + clip_thick - d_nub_tip - nub_r),
                       -nub_r - 1, nub_z - nub_len/2 - nub_r - 1])
                cube([nub_r*3 + 2, nub_r*2 + 2, nub_len + 2*nub_r + 2]);
        }
}

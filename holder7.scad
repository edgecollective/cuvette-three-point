// holder7.scad -- holder6 plus optional header cutaways in the shrouds.
//
// New in holder7: each shroud can get a slot through one lateral (+/-y)
...
//     to a board.
//
// PRINT UPSIDE DOWN (top face on the bed): the clip tongue then builds
// base-first from its anchor block and its enclosed pocket needs no support
// (which could never be removed).  Flipped, only the base plate's ear strips
// overhang -- let the slicer support just those; they're external and easy
// to remove.  The chamber floor (~13 mm) and shroud far segment (~35 mm)
// bridge acceptably.

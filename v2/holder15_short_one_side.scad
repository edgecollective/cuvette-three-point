// holder15_short_one_side.scad -- holder15 with ONE side's optical path
// shortened.  The chamber INTERNALS (cavity, rods, spring clip, pocket,
// walls around them) are exactly holder15's -- only the +x wall is
// thinner, and the plates/shrouds/base follow the asymmetric faces.
//
// Which board goes on the short side is your choice -- emitter and
// detector mounts are identical, so mount either PCB there (putting the
// DETECTOR close to the cuvette usually pays off most for light
// collection).  Identify sides by the built-in asymmetry (no labels
// needed):
//   SHORT side = +x = the wall carrying the TWO registration rods (look
//     into the chamber: two rods flank the window).  Freely thinnable --
//     no clip pocket in it.  wall_short = 2 mm puts that board 4 mm
//     closer to the cuvette than the 6 mm baseline.
//   LONG side  = -x = the side with the spring-clip corner (the pocket
//     lives in that wall, which stays at wall_width = 6, as in holder15).
//
// From holder15: slight bi-stability
...
holder_depth = 40;
wall_width   = 6;    // LATERAL (+/-y) wall thickness; also the default for
                     // an un-shortened optical-side wall (holder15 value)
...
outer_side   = holder_side + wall_width*2;   // lateral (y) outer width
window_z     = holder_depth/2;   // optical window height

/* ---- optical path length (per-side x-wall thickness) ---- */
short_wall = 2;   // +x wall thickness; the board there sits
                  // (wall_width - short_wall) closer to the cuvette

x_det  =  holder_side/2 + short_wall;    // +x outer face (short side)
x_emit = -(holder_side/2 + wall_width);  // -x outer face (clip side)
...
/* ---------- chamber body (chamber_depth tall, asymmetric in x) ---------- */
module body() {
    difference() {
        // chamber walls: per-side x faces, symmetric y faces
        translate([x_emit, -outer_side/2, 0])
            cube([x_det - x_emit, outer_side, chamber_depth]);
        ...
        translate([0, 0, window_z])
            rotate([0,90,0])
                cylinder(h=60, r=window_diam/2, center=true);
...
/* ---------- registration rods (drawn for clip at -x/+y) ---------- */
module rods() {
    // two rods along y on the +x cavity face
    translate([cuvette_dim/2+rod_radius, holder_side/3, 0])
        cylinder(h=chamber_depth, r=rod_radius);
    translate([cuvette_dim/2+rod_radius, -holder_side/3, 0])
        cylinder(h=chamber_depth, r=rod_radius);
    // one rod on the -y cavity face
...
/* ---------- base plate (asymmetric: follows the per-side plates) ---------- */
// x extents: flush with each shroud, but never leaving less than 4 mm of
// web around the board-mounting holes
base_x0 = min(x_emit - (plate_thick + shroud_h), -(mount_hole_x + 4));
base_x1 = max(x_det  + (plate_thick + shroud_h),  mount_hole_x + 4);

module base_plate() {
    difference() {
        // rounded-corner plate
        hull()
            for (xc=[base_x0 + base_r, base_x1 - base_r], sy=[-1,1])
                translate([xc, sy*(base_y/2 - base_r), 0])
    ...
    difference() {
        union() {
            ...
            translate([0,0,base_thick]) {
                body();
                rods();
                // per-side plates (mirror flips x only)
                translate([x_det, 0, 0])     // DETECTOR side (+x)
                    pcb_mount(header_cut=header_cutaway_det);
                translate([x_emit, 0, 0])    // EMITTER side (-x)
                    mirror([1,0,0]) pcb_mount(header_cut=header_cutaway_emit);
            }
        }
        // pocket subtracted from everything
        translate([0,0,base_thick]) clip_pocket();
    }
}

module holder15_short_one_side() {
    holder_shell();
...
// view, leaving the whole tongue standing exposed in the opening
module corner_reveal() {
    translate([x_emit - 4, 3, base_thick + 1])
        cube([-x_emit + 1, outer_side/2 + 1,
...
if (view_mode == "xray") {
    ...
    holder15_short_one_side();
}

// fit-check ghosts
if (show_ghosts) {
    translate([0,0,base_thick]) cuvette();
    translate([x_det, 0, base_thick]) pcb_ghost();
    translate([x_emit, 0, base_thick]) mirror([1,0,0]) pcb_ghost();
}
